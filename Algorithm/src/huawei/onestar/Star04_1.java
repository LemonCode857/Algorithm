...
//}
        ...
        String[] strAry = sc.nextLine().split("\\s+");
        sc.close();

        for (int i = 0; i < strAry.length;){
            int len = Integer.parseInt(strAry[i + 2] + strAry[i + 1],16);
            if (tag.equals(strAry[i])){
                StringBuilder sb = new StringBuilder();
                for (int x = i + 3; x < i + 3 + len; x++){
                    sb.append(strAry[x]).append(" ");
                }
